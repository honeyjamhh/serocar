...
<link rel="stylesheet" href="/resources/css/dealer/joinDealer.css" />

<body>
	<form action="./joinDealer" class="form" method="POST">
		<div class="center">
			<div class="container">
				...
				<div>
					<label class="label-text" for="dealerEmailTxt" id="dealerEmailTxt">이메일</label>
					<div class="uk-form-controls input-button-flex">
						<input class="uk-input" type="text" id="email"
							required="required" name="memberEmail" placeholder="" />
						<button type="button" class="uk-button email-btn"
							id="checkEmail" style="font-size: 12px">확인</button>
					</div>
				...
				<div>
					...
					<div class="uk-form-controls">
						<input class="uk-input" type="text" id="memberNickname"
							required="required" name="memberNickname" placeholder="" />
					</div>
				</div>
				...
				<div>
					...
					<div class="uk-form-controls">
						<input class="uk-input" type="password" id="memberPassword"
							required="required" name="memberPassword"
							placeholder="비밀번호(문자, 숫자, 특수문자 포함 10~20자)" onchange="check_pw()"
							required="required" onkeyup="passwordStrength()" />
					</div>
				...
				<div>
					...
					<div class="uk-form-controls">
						<input class="uk-input" type="password" id="passwordCheck"
							name="passwordCheck" required="required" placeholder="비밀번호 재입력"
							onchange="check_pw()" onkeyup="checkPassword()" /> <span
							id="checkPassword" style="font-size: 13px"></span>
					</div>
				</div>
				...
				<div>
					...
					<div class="uk-form-controls input-button-flex">
						<input type="text" name="memberBirth" id="memberBirth"
							required="required" placeholder="yymmdd" aria-label="birth"
							class="uk-input" maxlength="6" />
					</div>
				</div>

				<div>
					<label class="label-text" for="signInDealer">사업자 등록</label>
					<div class="uk-form-controls input-button-flex">
						<input class="uk-input" type="text" id="busniessNum" name="busniessNum"
							placeholder="사업자 등록번호" />
						<button type="button" class="uk-button email-btn"
							id="checkBusiness" style="font-size: 12px">확인</button>
					</div>
					<span style="font-size: 13px;" id="checkBusinessResult"></span>
				</div>
				...
				<div>
					<button class="myro-btn" id="completeJoin" type="submit">회원가입</button>
				</div>
				<div>
	...
	<script type="text/javascript">
		$(function() {
			...
			var busniessNum = $("#busniessNum").val();
			...
			$("#checkBusiness").click(function() {
				$.ajax({
					type : "get",
					url : "/checkBusiness",
					data : "busniessNum=" + $("#busniessNum").val(),
					success : function(result) {
						$("#checkBusinessResult").text(result);
					}
				});//ajax
			});//click 
		...
		/* 아이디 중복확인 */
			...
			$("#completeJoin").click(function(){
				...
				console.log("memberPasword : " + passwordCheck);
 				...
 				if ($("#checkBusinessResult").text() != "사용가능한 사업자 등록증입니다.") {
					alert("사업자 등록증 중복확인 하세요.");
					return false;
				} 
			});//click
		});//ready
		...
		function fn_emailChk(memberEmail) {
